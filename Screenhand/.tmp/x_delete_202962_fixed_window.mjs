...
const client = new Client({ name: 'x-delete-202962-fixed-window', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  const win = wins.find((w) => /ScreenHand on X:/i.test(w.title || ''))
    || wins.find((w) => /x\.com|\/ X|Home \/ X/i.test(w.title || ''))
    || wins.find((w) => !/about:blank/i.test(w.title || ''));
  if (!win) throw new Error('No suitable Chrome window');
  log.window = win;
  ...
  log.dialogShot = await call('screenshot', { windowId: win.windowId });
  log.confirmOffset = await call('click_text', { windowId: win.windowId, text: 'Delete post?', offset_y: 115 });
  await sleep(1000);
