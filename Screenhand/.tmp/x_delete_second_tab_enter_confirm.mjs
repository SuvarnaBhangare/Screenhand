...
const client = new Client({ name: 'x-delete-second-tab-enter-confirm', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  await sleep(350);
  log.tab1 = await call('key', { combo: 'tab' });
  await sleep(120);
  log.enter1 = await call('key', { combo: 'enter' });
  await sleep(250);
  log.enter2 = await call('key', { combo: 'enter' });
