...
const client = new Client({ name: 'x-delete-post-202962-confirm-dom', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  await sleep(500);
  log.clickDeleteMenu = await call('click_text', { windowId: win.windowId, text: 'Delete' });
  await sleep(500);

  log.confirmDeleteDom = await call('browser_js', {
    tabId: tab.id,
    code: `(() => {
      const clean = (s) => (s || '').replace(/\\s+/g, ' ').trim();
      const dialogs = Array.from(document.querySelectorAll('[role="dialog"]'));
      const dialog = dialogs.find((d) => /delete post/i.test(clean(d.innerText || ''))) || dialogs[0] || null;
      if (!dialog) return { ok:false, reason:'delete-dialog-not-found' };
      ...
      const del = btns.find((b) => {
        ...
        return !disabled && /^delete$/i.test(label);
      });
      if (!del) {
        return { ok:false, reason:'delete-button-not-found', labels: btns.map((b) => clean(b.textContent || '') || clean(b.getAttribute('aria-label') || '')).slice(0, 10) };
      }
      del.click();
      return { ok:true, clicked: clean(del.textContent || '') || clean(del.getAttribute('aria-label') || '') };
    })()`
  });
  ...
  log.verifyDom = await call('browser_js', {
    tabId: tab.id,
    code: `(() => {
      const txt = (document.body?.innerText || '').replace(/\\s+/g, ' ').trim();
      return {
        deletedLike: /deleted|post not found|doesn.?t exist|try searching/i.test(txt),
