import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StdioClientTransport } from '@modelcontextprotocol/sdk/client/stdio.js';

const URL = 'https://x.com/screenhand_/status/2029617823178424794';
const NEW_TEXT = 'Updated by ScreenHand team: AI agents can automate real Mac workflows end-to-end with OCR, native app control, and Chrome automation via MCP. Learn more: https://screenhand.com #AIAgents #MCP #macOS #ScreenHand';

const transport = new StdioClientTransport({ command: 'npx', args: ['tsx', '/Users/khushi/Documents/Automator/Screenhand/mcp-desktop.ts'] });
const client = new Client({ name: 'x-edit-post-202961-via-ocr', version: '1.0.0' }, { capabilities: {} });
const t = (r) => r?.content?.find?.((c) => c.type === 'text')?.text || JSON.stringify(r);
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function parseTabs(text) {
  return (text || '').split('\n').map((line) => {
    const m = line.match(/^\[([^\]]+)\]\s*(.*?)\s+—\s+(.*)$/);
    return m ? { id: m[1], title: m[2], url: m[3] } : null;
  }).filter(Boolean);
}

function parseWindows(text) {
  return (text || '').split('\n').map((line) => {
    const m = line.match(/^\[(\d+)\]\s+(.*?)\s+"(.*)"\s+\(([-\d.]+),([-\d.]+)\)\s+(\d+)x(\d+)$/);
    if (!m) return null;
    return { windowId: Number(m[1]), appName: m[2], title: m[3], area: Number(m[6]) * Number(m[7]) };
  }).filter(Boolean);
}

async function call(name, args = {}) {
  try {
    const res = await client.callTool({ name, arguments: args });
    return { ok: true, text: t(res) };
  } catch (err) {
    return { ok: false, error: String(err?.message || err) };
  }
}

try {
  await client.connect(transport);
  const log = { newText: NEW_TEXT };
  log.focus = await call('focus', { bundleId: 'com.google.Chrome' });
  const tabs = parseTabs((await call('browser_tabs', {})).text || '');
  const tab = tabs.find((x) => /(x\.com|twitter\.com)/i.test(x.url));
  if (!tab) throw new Error('No X tab');

  log.nav = await call('browser_navigate', { tabId: tab.id, url: URL });
  log.wait = await call('browser_wait', { tabId: tab.id, condition: 'document.body && document.body.innerText.length > 100', timeoutMs: 30000 });

  log.openMenu = await call('browser_js', {
    tabId: tab.id,
    code: `(() => {
      const link = Array.from(document.querySelectorAll('a[href]')).find((a) => (a.getAttribute('href') || '').includes('/status/2029617823178424794'));
      const article = link ? link.closest('article') : document.querySelector('article');
      const caret = article?.querySelector('[data-testid="caret"], button[aria-label="More"]');
      if (!caret) return { ok:false, reason:'no-caret' };
      caret.click();
      return { ok:true };
    })()`
  });

  const wins = parseWindows((await call('windows', {})).text || '').filter((w) => /Google Chrome/i.test(w.appName));
  const win = wins.find((w) => /screenhand|x\s*:/i.test(w.title || '')) || wins.sort((a,b)=>b.area-a.area)[0];
  if (!win) throw new Error('No Chrome window');
  log.window = win;

  await sleep(500);
  log.menuShot = await call('screenshot', { windowId: win.windowId });

  log.clickEdit = await call('click_text', { windowId: win.windowId, text: 'Edit' });
  await sleep(700);

  // Try direct DOM replace in editor
  log.domReplace = await call('browser_js', {
    tabId: tab.id,
    code: `(() => {
      const text = ${JSON.stringify(NEW_TEXT)};
      const clean = (s) => (s || '').replace(/\\s+/g, ' ').trim();
      const dialogs = Array.from(document.querySelectorAll('[role="dialog"]'));
      const dialog = dialogs.find((d) => d.querySelector('[data-testid="tweetTextarea_0"], div[role="textbox"][contenteditable="true"]')) || null;
      const root = dialog || document;
      const box = root.querySelector('[data-testid="tweetTextarea_0"], div[role="textbox"][contenteditable="true"]');
      if (!box) return { ok:false, reason:'editor-not-found' };
      box.focus();
      const sel = window.getSelection();
      const range = document.createRange();
      range.selectNodeContents(box);
      sel.removeAllRanges();
      sel.addRange(range);
      document.execCommand('insertText', false, text);
      const val = clean(box.innerText || box.textContent || '');
      return { ok:true, len: val.length, preview: val.slice(0, 200) };
    })()`
  });

  // Fallback keyboard clear/type
  if ((log.domReplace.text || '').includes('editor-not-found')) {
    log.selectAll = await call('key', { combo: 'cmd+a' });
    await sleep(120);
    log.backspace = await call('key', { combo: 'backspace' });
    await sleep(150);
    log.type = await call('type_text', { text: NEW_TEXT });
  }

  await sleep(250);

  // Strict submit via Update/Save only (never Reply)
  log.submitDom = await call('browser_js', {
    tabId: tab.id,
    code: `(() => {
      const clean = (s) => (s || '').replace(/\\s+/g, ' ').trim();
      const dialogs = Array.from(document.querySelectorAll('[role="dialog"]'));
      const dialog = dialogs.find((d) => d.querySelector('[data-testid="tweetTextarea_0"], div[role="textbox"][contenteditable="true"]')) || null;
      const root = dialog || document;
      const buttons = Array.from(root.querySelectorAll('button,[role="button"]'));
      const update = buttons.find((b) => {
        const label = clean(b.textContent || '') + ' ' + clean(b.getAttribute('aria-label') || '');
        const disabled = !!b.disabled || b.getAttribute('aria-disabled') === 'true';
        return !disabled && /update|save/i.test(label);
      });
      const reply = buttons.find((b) => /reply/i.test(clean(b.textContent || '') + ' ' + clean(b.getAttribute('aria-label') || '')));
      if (!update) return { ok:false, reason:'update-save-not-found', hasReply: !!reply };
      const label = clean(update.textContent || '') || clean(update.getAttribute('aria-label') || '');
      update.click();
      return { ok:true, clicked: label };
    })()`
  });

  // OCR fallback click Update/Save only
  if ((log.submitDom.text || '').includes('update-save-not-found')) {
    log.clickUpdate = await call('click_text', { windowId: win.windowId, text: 'Update' });
    if ((log.clickUpdate.text || '').includes('not found')) {
      log.clickSave = await call('click_text', { windowId: win.windowId, text: 'Save' });
    }
  }

  await sleep(1100);
  log.verifyNav = await call('browser_navigate', { tabId: tab.id, url: URL });
  log.verifyWait = await call('browser_wait', { tabId: tab.id, condition: 'document.body && document.body.innerText.length > 60', timeoutMs: 30000 });
  log.verifyShot = await call('screenshot', { windowId: win.windowId });

  console.log(JSON.stringify({ ok: true, log }, null, 2));
} catch (err) {
  console.log(JSON.stringify({ ok: false, error: String(err?.message || err) }, null, 2));
  process.exitCode = 1;
} finally {
  try { await client.close(); } catch {}
}
