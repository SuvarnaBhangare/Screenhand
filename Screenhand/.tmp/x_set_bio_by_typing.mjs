...
const OUT = '/Users/khushi/Documents/Automator/Screenhand/.tmp/x_set_bio_by_typing_report.json';
const BIO = 'Open-source MCP server for desktop automation.';

const transport = new StdioClientTransport({ command: 'npx', args: ['tsx', '/Users/khushi/Documents/Automator/Screenhand/mcp-desktop.ts'] });
const client = new Client({ name: 'x-set-bio-by-typing', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  // Focus bio with AX, then type via real keyboard events
  report.steps.push({ step: 'pressBio', result: await call('ui_press', { pid: chrome.pid, title: 'Bio' }) });
  ...
  const a = afterSaveShot.text || '';
  report.afterSave = {
    modalStillVisible: /Edit your photo with Imagine|Bio\s+\d+\s*\/\s*160|Save/i.test(a),
    hasBioInModal: /open-source mcp server for desktop automation/i.test(a.toLowerCase()),
  ...
  report.steps.push({ step: 'publicVerify', result: report.public });
