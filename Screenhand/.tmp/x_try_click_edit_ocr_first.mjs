...
const client = new Client({ name: 'x-try-click-edit-ocr-first', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  log.openMenu = await call('browser_js', {
    ...
    code: `(() => {
      ...
      const caret = article?.querySelector('[data-testid="caret"], button[aria-label="More"]') || document.querySelector('[data-testid="caret"], button[aria-label="More"]');
  ...
  const win = wins.find((w) => /ScreenHand on X:|\/ X|Home \/ X|\bx\b/i.test(w.title || '') && !/about:blank/i.test(w.title || '')) || wins.sort((a,b)=>b.area-a.area)[0];
