...
const client = new Client({ name: 'x-shot-tweet-new', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  const win = wins.find((w) => /Google Chrome/i.test(w.appName) && /x|twitter|screenhand/i.test(w.title) && !/about:blank/i.test(w.title))
  ...
  console.log(JSON.stringify({ ok: true, window: win, shot: shot.slice(0, 3000) }, null, 2));
} catch (err) {
