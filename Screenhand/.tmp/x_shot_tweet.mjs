...
const client = new Client({ name: 'x-shot-tweet', version: '1.0.0' }, { capabilities: {} });
...
function parseWindows(text) {
  return (text || '').split('\n').map((line) => {
    ...
    if (!m) return null;
    return { windowId: Number(m[1]), appName: m[2], title: m[3], w: Number(m[6]), h: Number(m[7]), raw: line };
  }).filter(Boolean);
}
...
try {
  ...
  const win = wins.find((w) => /Google Chrome/i.test(w.appName) && /x|twitter/i.test(w.title) && !/about:blank/i.test(w.title))
    || wins.find((w) => /Google Chrome/i.test(w.appName) && !/about:blank/i.test(w.title));
  if (!win) throw new Error('No Chrome X window');

  const shot = await call('screenshot', { windowId: win.windowId });
  console.log(JSON.stringify({ ok: true, window: win, shot: shot.slice(0, 2600) }, null, 2));
} catch (err) {
