...
const client = new Client({ name: "screenhand-devpost-captcha-click-text", version: "1.0.0" }, { capabilities: {} });
...
try {
  ...
  let windows = [];
  try { windows = JSON.parse(windowsText); } catch {}

  const chromeWin = windows.find(w => String(w.app || w.appName || "").toLowerCase().includes("chrome")) || windows[0];
  const windowId = chromeWin?.windowId;

  let clickResult = "No Chrome window found";
