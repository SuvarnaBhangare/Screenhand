...
const OUT = '/Users/khushi/Documents/Automator/Screenhand/.tmp/x_set_bio_simple_debug_report.json';
const BIO = 'Open-source MCP server for desktop automation.';

const transport = new StdioClientTransport({ command: 'npx', args: ['tsx', '/Users/khushi/Documents/Automator/Screenhand/mcp-desktop.ts'] });
const client = new Client({ name: 'x-set-bio-simple-debug', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  const afterSaveShot = await call('screenshot', { windowId: chromeWin.windowId });
  const afterSaveText = afterSaveShot.text || '';
  report.afterSave = {
    modalStillVisible: /Edit your photo with Imagine|Bio\s+\d+\s*\/\s*160|Save/i.test(afterSaveText),
    hasError: /try again|couldn't|problem saving|error/i.test(afterSaveText.toLowerCase()),
    snippet: afterSaveText.slice(0, 2600)
  };
  ...
  const publicShot = await call('screenshot', { windowId: chromeWin.windowId });
  const p = publicShot.text || '';
  report.public = {
  ...
  report.steps.push({ step: 'publicShot', result: report.public });

  report.finishedAt = new Date().toISOString();
  fs.writeFileSync(OUT, JSON.stringify(report, null, 2));
  console.log(JSON.stringify({ ok: true, out: OUT, afterSave: report.afterSave, public: report.public }, null, 2));
} catch (err) {
